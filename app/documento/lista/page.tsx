...
const ListarDocumento = () => {
  const [documentos, setDocumentos] = useState<DadosItem[] | null>(null);
  ...
  const handleConfirmDelete = async () => {
    try {
      await helpers.deleteDocumento(selectedItemId);
      if (documentos) {
        const updatedDocumentos = documentos.filter(
          (documento: DadosItem) => documento.id !== selectedItemId
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resposta = await helpers.getDocumentos();
        // setDocumentos(resposta);
        setLoading(false);
      } catch (erro: any) {
        console.error("Erro ao obter documentos:", erro.message);
        setLoading(false);
      }
    };
        ...
        <div className="overflow-x-auto mt-4">
          {loading ? (
            ...
            <table className="bg-white border border-gray-300">
              ...
              <tbody>
                {documentos &&
                  documentos.map((row: any, index: any) => (
                    <tr
                      key={row.id}
